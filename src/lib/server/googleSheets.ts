...
const definitions: Record<string, { headers: string[]; values: (payload: any) => unknown[] }> = {
	...
	Tasks: { headers: ['Task ID', 'Order ID', 'Task', 'Editor ID', 'Editor Name', 'Due Date', 'Task Value (Billable)', 'Already Invoiced', 'Instructions', 'Text Link', 'Image URL', 'Status', 'Progress', 'Output Link', 'Notes', 'Archived', 'Record ID'], values: (task) => [task.id, task.orderId, task.name, task.editorCode || '', task.assignee || '', task.due || '', task.billableAmount || 0, task.invoicedAmount || 0, task.instructions || '', task.textLink || '', task.imageUrl || '', task.status, task.progress, task.outputLink || '', task.notes || '', task.archived ? 'Yes' : 'No', task.id] },
