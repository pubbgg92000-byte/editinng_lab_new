...
export async function resetDemoTenant(tenant: Tenant) {
	...
	await database.batch([
		...
		].map((customer, index) => database.prepare('INSERT INTO customers (id, name, business, phone, email, address, gst, portal_token_hash, portal_token_cipher, projects, pending, created_at, updated_at, archived_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?, ?, ?)')
			.bind(customer[0], customer[1], customer[2], customer[3], customer[4], 'Demo address', customer[6], customerTokens[index].hash, customerTokens[index].cipher, now, now, customer[5] || null)),
		...
		].map((editor, index) => database.prepare('INSERT INTO editors (id, code, name, phone, specialty, availability, portal_token_hash, portal_token_cipher, created_at, updated_at, archived_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
			.bind(editor[0], editor[1], editor[2], editor[3], editor[4], editor[5], editorTokens[index].hash, editorTokens[index].cipher, now, now, editor[6])),
		...
		...[
			['TSK-DEMO-001', 'ORD-DEMO-001', null, 'Organise footage', 'Not started', 0, 0, ''],
			['TSK-DEMO-002', 'ORD-DEMO-002', 'ED-DEMO-001', 'Cinematic teaser', 'Files downloaded', 10, 12000, ''],
			['TSK-DEMO-003', 'ORD-DEMO-003', 'ED-DEMO-002', 'Main wedding film', 'In progress', 45, 45000, ''],
			['TSK-DEMO-004', 'ORD-DEMO-004', 'ED-DEMO-001', 'Reception highlights', 'Ready for review', 75, 25000, ''],
			['TSK-DEMO-005', 'ORD-DEMO-005', 'ED-DEMO-002', 'Revision pass', 'Revision required', 82, 18000, ''],
			['TSK-DEMO-006', 'ORD-DEMO-006', 'ED-DEMO-003', 'Birthday highlight', 'Completed', 100, 20000, 'https://example.com/demo-output'],
			['TSK-DEMO-007', 'ORD-DEMO-007', 'ED-DEMO-001', 'Archive delivery', 'Completed', 100, 35000, 'https://example.com/demo-delivery'],
			['TSK-DEMO-008', 'ORD-DEMO-003', null, 'Social media reel', 'Not started', 0, 8000, '']
		].map((task) => database.prepare('INSERT INTO tasks (id, order_id, editor_id, title, instructions, due_date, text_link, image_url, status, progress, output_link, notes, billable_amount, created_at, updated_at, archived_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)')
			.bind(task[0], task[1], task[2], task[3], 'Use this record to explore the complete workflow.', '2026-08-10', 'https://example.com/demo-reference', '', task[4], task[5], task[7], '', task[6], now, now)),
