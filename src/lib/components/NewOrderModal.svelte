...
<script lang="ts">
	...
	import type { Customer, CustomFieldValue, Order } from '$lib/types';
	...
	type Draft = {
		...
		amount?: number; advance?: number; source: string; due: string; remarks: string; important: boolean; tasks: TaskDraft[]; customFields: Record<string, CustomFieldValue>;
	};

	let { open = $bindable(), onsaved = () => {} }: { open: boolean; onsaved?: (order: Order, synced: boolean) => void } = $props();
	const editingProfile = $derived(page.data.configuration?.profile?.presetId === 'editing-studio');
	const sourceOptions = $derived(editingProfile ? ['HD-1', 'HD-2', 'Customer drive', 'Online transfer', 'Other'] : ['Walk-in', 'Phone', 'WhatsApp', 'Referral', 'Online', 'Other']);
	const configuredCategories = $derived(page.data.configuration?.profile?.optionLists?.categories || ['General']);
	const customerLabel = $derived(labelFor(page.data.configuration?.profile, 'customer'));
	const orderLabel = $derived(labelFor(page.data.configuration?.profile, 'order'));
	const taskLabel = $derived(labelFor(page.data.configuration?.profile, 'task'));
	const categoryLabel = $derived(labelFor(page.data.configuration?.profile, 'category'));
	const projectLabel = $derived(labelFor(page.data.configuration?.profile, 'project'));
	const dueLabel = $derived(labelFor(page.data.configuration?.profile, 'dueDate'));
	const tasksEnabled = $derived(hasCapability(page.data.configuration?.effectiveCapabilities, 'work.tasks'));
	const paymentsEnabled = $derived(hasCapability(page.data.configuration?.effectiveCapabilities, 'billing.payments'));
	...
	const newDraft = (): Draft => ({ key: `order-draft-${++draftCounter}`, event: configuredCategories[0] || 'General', customEvent: '', project: '', receiving: '', duration: '', source: sourceOptions[0], due: '', remarks: '', important: false, tasks: [newTask()], customFields: {} });

	let customerId = $state('');
	let mobile = $state('');
	let showCustomer = $state(false);
	let drafts = $state<Draft[]>([newDraft()]);
	let eventOptions = $state<EventOption[]>([]);
	...
	$effect(() => {
		if (open && !loadedForOpen) {
			loadedForOpen = true;
			eventOptions = configuredCategories.map((name: string) => ({ name, custom: false }));
	...
	async function loadEventOptions() {
		loadingEvents = true;
		try {
			const result = await requestJson<{ options: EventOption[] }>('/api/event-options', {}, 'Unable to load saved event types.');
			const custom = result.options.filter((option) => option.custom);
			eventOptions = [...configuredCategories.map((name: string) => ({ name, custom: false })), ...custom.filter((option) => !configuredCategories.includes(option.name))];
		} catch (cause) {
			eventOptions = configuredCategories.map((name: string) => ({ name, custom: false }));
			error = cause instanceof Error ? cause.message : 'Unable to load saved event types.';
	...
	async function addCustomEvent(draft: Draft) {
		...
		try {
			const result = await requestJson<{ options: EventOption[] }>('/api/event-options', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name }) }, 'Unable to add event type.');
			eventOptions = result.options;
			draft.event = result.options.find((item) => item.name.toLowerCase() === name.toLowerCase())?.name || name;
			draft.customEvent = '';
			error = '';
		} catch (cause) {
			error = cause instanceof Error ? cause.message : 'Unable to add event type.';
		}
	}
	...
	async function deleteCustomEvent(name: string) {
		...
		try {
			const result = await requestJson<{ options: EventOption[] }>('/api/event-options', { method: 'DELETE', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name }) }, 'Unable to remove event type.');
			eventOptions = result.options;
			for (const draft of drafts) if (draft.event === name) draft.event = configuredCategories[0] || 'General';
		} catch (cause) {
			error = cause instanceof Error ? cause.message : 'Unable to remove event type.';
		}
	}

	async function save() {
		if (!selectedCustomer) { error = `Choose or create a ${customerLabel.toLowerCase()}.`; return; }
		...
		for (const [index, draft] of drafts.entries()) {
			if (!draft.event.trim() || draft.event === 'Other' || !draft.project.trim()) { error = `Add the ${categoryLabel.toLowerCase()} and ${projectLabel.toLowerCase()} for ${orderLabel.toLowerCase()} ${index + 1}.`; return; }
		...
		error = '';
		try {
			const result = await requestJson<{ orders?: Order[]; order?: Order; sync?: { configured?: boolean; failed?: number } }>('/api/orders', {
			...
			}, `Unable to create ${labelFor(page.data.configuration?.profile, 'order', true).toLowerCase()}.`);
			const createdOrders = result.orders ?? (result.order ? [result.order] : []);
			if (!createdOrders.length) throw new Error('The server did not return the created order.');
			for (const order of createdOrders) onsaved(order, result.sync?.configured === true && result.sync?.failed === 0);
			open = false;
			reset();
...
<Modal title={drafts.length > 1 ? `Create ${drafts.length} ${labelFor(page.data.configuration?.profile, 'order', true).toLowerCase()}` : `Create new ${orderLabel.toLowerCase()}`} bind:open wide>
	<p class="sheet-note">Choose the {customerLabel.toLowerCase()} once, then add one or more {labelFor(page.data.configuration?.profile, 'project', true).toLowerCase()}. Each becomes a separate {orderLabel.toLowerCase()}.</p>
	<div class="form-grid customer-fields">
		<div class="field"><label for="order-studio">{customerLabel} <RequiredMark/></label><div class="customer-select"><div class="select-control"><select id="order-studio" bind:value={customerId} required aria-required="true"><option value="">Choose {customerLabel.toLowerCase()}</option>{#each $customerStore as item}<option value={item.id}>{item.business}</option>{/each}</select><span class="select-chevron" aria-hidden="true"><ChevronDown size={15}/></span></div><button class="secondary" type="button" onclick={() => (showCustomer = true)}>New</button></div></div>
		<div class="field"><label for="order-mobile">10-digit mobile number <RequiredMark/></label><input id="order-mobile" bind:value={mobile} inputmode="tel" autocomplete="tel" placeholder="98765 43210" required aria-required="true" /></div>
	</div>

	<div class="orders-stack">
		{#each drafts as draft, index (draft.key)}
			<section class="order-draft">
				<div class="draft-head"><div><strong>{orderLabel} {index + 1}</strong><span>{draft.project || `New ${projectLabel.toLowerCase()}`}</span></div>{#if drafts.length > 1}<button type="button" class="remove-order" title={`Remove this ${orderLabel.toLowerCase()}`} onclick={() => removeOrder(index)}><Trash2 size={14}/></button>{/if}</div>
				<div class="form-grid">
					<div class="field"><label for={`order-event-${draft.key}`}>{categoryLabel} <RequiredMark/></label><div class="select-control"><select id={`order-event-${draft.key}`} bind:value={draft.event} disabled={loadingEvents} required aria-required="true">{#each eventOptions as option}<option value={option.name}>{option.name}</option>{/each}<option value="Other">+ Other / create new</option></select><span class="select-chevron" aria-hidden="true"><ChevronDown size={15}/></span></div></div>
					<div class="field"><label for={`order-name-${draft.key}`}>{projectLabel} <RequiredMark/></label><input id={`order-name-${draft.key}`} bind:value={draft.project} required aria-required="true" /></div>
					...
					<div class="field"><label for={`order-due-${draft.key}`}>{dueLabel}</label><input id={`order-due-${draft.key}`} bind:value={draft.due} type="date" /></div>
					...
					{#if paymentsEnabled}<div class="field"><label for={`order-advance-${draft.key}`}>Advance collected (₹) <span>Optional</span></label><input id={`order-advance-${draft.key}`} bind:value={draft.advance} type="number" min="0" step="1" placeholder="0" /></div>{/if}
				...
				{#if customFieldsEnabled}<CustomFields definitions={page.data.configuration.profile.customFields} entity="order" bind:values={draft.customFields}/>{/if}
				{#if tasksEnabled}<div class="field task-input"><div class="task-heading"><span class="task-label">{labelFor(page.data.configuration?.profile, 'task', true)} <small>Optional</small></span><button type="button" onclick={() => addTask(draft)}><Plus size={13}/> Add {taskLabel.toLowerCase()}</button></div><div class="task-drafts">{#each draft.tasks as task, taskIndex (task.key)}<div class="task-draft"><input aria-label={`${taskLabel} ${taskIndex + 1} name`} bind:value={task.name}/><input aria-label={`${taskLabel} ${taskIndex + 1} value`} bind:value={task.billableAmount} type="number" min="0" placeholder="Value ₹"/><button type="button" aria-label={`Remove ${taskLabel.toLowerCase()} ${taskIndex + 1}`} onclick={() => removeTask(draft, taskIndex)}><X size={13}/></button></div>{/each}</div><small>Values are optional. Completed work with a value can be billed partially.</small></div>{/if}
	...
	<button type="button" class="add-order" onclick={addOrder}><Plus size={15}/> Add another {orderLabel.toLowerCase()} for this {customerLabel.toLowerCase()}</button>
	...
	{#snippet footer()}<button type="button" class="secondary" disabled={saving} onclick={() => (open = false)}>Cancel</button><button type="button" class="primary" disabled={saving} aria-busy={saving} onclick={save}>{saving ? 'Creating…' : drafts.length > 1 ? `Create ${drafts.length} ${labelFor(page.data.configuration?.profile, 'order', true).toLowerCase()}` : `Create ${orderLabel.toLowerCase()}`}</button>{/snippet}
</Modal>
