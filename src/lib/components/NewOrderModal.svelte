<script lang="ts">
	...
	import { Plus, Star, Trash2, X } from 'lucide-svelte';
...
<Modal title={drafts.length > 1 ? `Create ${drafts.length} orders` : 'Create new order'} bind:open wide>
	...
	<div class="form-grid customer-fields">
		<div class="field"><label for="order-studio">Studio name *</label><div class="customer-select"><select id="order-studio" bind:value={customerId}><option value="">Choose customer</option>{#each $customerStore as item}<option value={item.id}>{item.business}</option>{/each}</select><button class="secondary" type="button" onclick={() => (showCustomer = true)}>New</button></div></div>
	...
	<div class="orders-stack">
		{#each drafts as draft, index (draft.key)}
			<section class="order-draft">
				...
				<div class="form-grid">
					<div class="field"><label for={`order-event-${draft.key}`}>Event *</label><select id={`order-event-${draft.key}`} bind:value={draft.event} disabled={loadingEvents}>{#each eventOptions as option}<option value={option.name}>{option.name}</option>{/each}<option value="Other">+ Other / create new</option></select></div>
					...
					<div class="field"><label for={`order-source-${draft.key}`}>Source</label><select id={`order-source-${draft.key}`} bind:value={draft.source}>{#each sourceOptions as option}<option>{option}</option>{/each}</select></div>
