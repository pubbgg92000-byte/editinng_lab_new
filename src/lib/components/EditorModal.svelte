<script lang="ts">
	...
	async function save() { if (!name.trim()) { error='Editor name is required.'; return; } const phoneError=indianMobileError(phone,true); if(phoneError){error=phoneError;return} phone=normalizeIndianMobile(phone); saving=true; const response=await fetch(editor?`/api/editors/${editor.id}`:'/api/editors',{method:editor?'PATCH':'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name,phone,specialty,availability})}); const result=await response.json(); saving=false; if(!response.ok){error=result.error||'Unable to save editor';return} onsaved(result.editor); open=false; }
</script>
