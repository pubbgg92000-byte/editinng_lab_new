...
<script lang="ts">
	...
	async function deliver() {
		...
		busy = true; error = '';
		try {
			const result = await requestJson<{ order: Order }>(`/api/orders/${order.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ status: 'Delivered', progress: 100, deliveryMethod: method, deliveredAt: new Date().toISOString() }) }, 'Unable to mark this order delivered.');
			open = false;
			ondelivered?.(result.order);
			notifyAction('Delivery confirmed and saved.');
		} catch (cause) {
			error = cause instanceof Error ? cause.message : 'Unable to mark this order delivered.';
		} finally {
			busy = false;
		}
	}
