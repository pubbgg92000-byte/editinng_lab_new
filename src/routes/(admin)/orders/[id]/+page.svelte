<script lang="ts">
	...
	import { ArrowLeft, Archive, Check, ChevronRight, Edit3, ExternalLink, FileText, FolderKanban, HardDrive, IndianRupee, Plus, RotateCcw, Star, Settings2, PackageCheck, CircleCheckBig } from '@lucide/svelte';
	...
	const customerRecord = $derived(data.customers.find((customer: { id: string }) => customer.id === order.customerId));
	const customerOrders = $derived((data.customerOrders as Order[]).filter((item) => !item.archived));
	...
	function openBill() { error = ''; invoiceModalOpen = true; }
...
<div class="detail-top">
	...
	<div class="actions">
		{#if !order.archived}<button class="secondary" onclick={() => billingModalOpen = true}><Settings2 size={14}/> Set order total</button><button class="secondary" onclick={() => paymentModalOpen = true}><IndianRupee size={14}/> Record customer payment</button>
		<button class="primary" disabled={busy === 'invoice'} onclick={openBill}><FileText size={15}/> {busy === 'invoice' ? 'Creating final invoice…' : 'Create final invoice'}</button><button class="archive-order" disabled={busy==='archive'} onclick={archiveCurrentOrder}><Archive size={14}/> Archive this order</button>{/if}
	</div>
</div>
...
<header class="order-heading">
	...
	<div>
		...
		<div class="customer-heading">{#if customerRecord}<a href={`/customers?customer=${customerRecord.id}`}>{order.customer}</a>{:else}<strong>{order.customer}</strong>{/if}<span>{order.workType || 'Event not set'}</span></div>
	</div>
</header>
...
<div class="detail-grid">
				...
				<div class="task-list">
					{#each visibleTasks as task}
						<article class:archived-task={task.archived} class="task-row">
							<div class="task-title"><strong>{task.name}</strong><small>{#if task.editorId}<a class="editor-link" href={`/editors?editor=${task.editorId}`}>{task.assignee}</a>{:else}{task.assignee}{/if} {task.due ? `· Due ${formatDate(task.due)}` : ''}</small><div class="task-meta">{#if task.device}<span><HardDrive size={11}/> {task.device}</span>{/if}{#if task.billingMode === 'duration'}<span>{formatVideoDuration(task.videoDurationMinutes) || 'Duration pending'} · {money(task.hourlyRate || 0)}/hr</span>{/if}</div>{#if (task.billableAmount || 0) > 0}<small class="task-billing">Billing amount {money(task.billableAmount || 0)}{#if (task.invoicedAmount || 0) > 0} · Invoiced {money(task.invoicedAmount || 0)}{/if}</small>{/if}</div>
	...
	<div class="side-col">
		<section class="card customer-orders-card">
			<div class="customer-card-head"><span><FolderKanban size={16}/></span><div><small>Customer / studio</small>{#if customerRecord}<a href={`/customers?customer=${customerRecord.id}`}>{order.customer}</a>{:else}<strong>{order.customer}</strong>{/if}<p>{customerOrders.length} {customerOrders.length === 1 ? 'order' : 'orders'} in this account</p></div></div>
			{#if customerOrders.length}<div class="customer-order-list">{#each customerOrders as customerOrder}<a class:current={customerOrder.id === order.id} href={`/orders/${customerOrder.id}`}><span><strong>{customerOrder.project}</strong><small>#{customerOrder.serial} · {customerOrder.workType}</small></span><span class="order-state">{customerOrder.id === order.id ? 'Current' : customerOrder.status}</span><ChevronRight size={14}/></a>{/each}</div>{:else}<p class="muted">No other orders are linked to this customer.</p>{/if}
		</section>
		...
		<section class="card invoice-card">
			...
			<div class="billing-buttons"><button class="invoice-button" onclick={() => billingModalOpen = true}><Settings2 size={13}/> {order.priceSet===false?'Set order total':'Edit order total'}</button><button class="invoice-button" onclick={() => paymentModalOpen = true}><IndianRupee size={13}/> Record payment</button></div>
		</section>
	</div>
</div>

<TaskModal bind:open={taskModalOpen} orderId={order.id} bind:editors bind:devices task={editingTask} onsaved={refresh}/>
