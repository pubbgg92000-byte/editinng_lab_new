<script lang="ts">
	...
	function editTemplate(type: 'assignmentTemplate' | 'invoiceTemplate') {
		...
		if (type === 'invoiceTemplate' && !templateDraft.includes('{{portal_link}}')) templateDraft = `${templateDraft.trimEnd()}\n\nView work status and bill:\n{{portal_link}}`;
		templateModalOpen = true;
	}

	async function saveTemplate() {
		templateSaving = true;
		const savedTemplate = editingTemplate === 'invoiceTemplate' && !templateDraft.includes('{{portal_link}}') ? `${templateDraft.trimEnd()}\n\nView work status and bill:\n{{portal_link}}` : templateDraft;
		const response = await fetch('/api/settings', { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ [editingTemplate]: savedTemplate }) });
	...
	async function run(action: 'sync' | 'import') {
		...
		try {
			...
			else if (action === 'import') message = `Imported ${result.imported} orders; skipped ${result.skipped}. ${result.editorsArchived ? `${result.editorsArchived} editor(s) missing from Sheets moved to Archived.` : 'Editors are reconciled.'}`;
...
<div class="settings-grid">
	<section class="card settings-card">
		...
		<div class="form-grid">
			...
			<div class="field"><label for="studio-logo">HTTPS logo URL</label><input id="studio-logo" type="url" bind:value={settings.logoUrl}/></div>
	...
	<section class:critical={data.storage.level === 'critical'} class:warning={data.storage.level === 'warning'} class:notice={data.storage.level === 'notice'} class="card storage-card collapsible-card">
		<button class="storage-heading dropdown-heading" aria-expanded={storageOpen} onclick={() => (storageOpen = !storageOpen)}><span><Database size={17}/></span><div><h2>Database storage</h2><p>{data.storage.percent}% used · Open only when you need storage details.</p></div><strong>{data.storage.percent}%</strong><ChevronDown class={storageOpen ? 'open' : ''} size={17}/></button>
		{#if storageOpen}<div class="dropdown-body"><div class="storage-meter"><span style:width={`${Math.min(100, data.storage.percent)}%`}></span></div>
		<div class="storage-numbers"><span>{formatBytes(data.storage.bytes)} used</span><span>{data.storage.limitMb} MB monitored limit</span></div>
		<div class="retention-status"><strong>Automatic retention</strong><span>Successful Sheet-sync records: 30 days</span><span>Activity logs: 12 months</span>{#if data.storage.cleanup.lastRunAt}<small>Last checked {formatDateTime(data.storage.cleanup.lastRunAt)}</small>{/if}</div></div>{/if}
	</section>

	<section class="card template-card collapsible-card">
		<button class="dropdown-heading template-heading" aria-expanded={templatesOpen} onclick={() => (templatesOpen = !templatesOpen)}><span><MessageSquareText size={17}/></span><div><h2>WhatsApp templates</h2><p>Editor assignments and customer status/bill messages.</p></div><ChevronDown class={templatesOpen ? 'open' : ''} size={17}/></button>
		{#if templatesOpen}<div class="template-grid dropdown-body">
			...
			<article class="template-preview"><div class="template-title"><span><MessageSquareText size={16}/></span><div><strong>Customer bill & status</strong><small>Invoice, payments and unique private status link</small></div><button onclick={() => editTemplate('invoiceTemplate')} aria-label="Edit customer bill and status template"><Pencil size={14}/></button></div><pre>{settings.invoiceTemplate}</pre></article>
		</div>{/if}
	</section>
...
<Modal title={editingTemplate === 'assignmentTemplate' ? 'Edit editor assignment template' : 'Edit customer bill & status template'} bind:open={templateModalOpen} wide>
	<div class="template-editor"><p>Keep placeholder names inside double braces. Messages open in WhatsApp for review before sending.</p><textarea bind:value={templateDraft}></textarea><small>{editingTemplate === 'assignmentTemplate' ? 'Available: editor_name, studio_name, project, customer, task_list, portal_link' : 'Available: studio_name, studio_address, studio_phone_line, gstin_line, invoice_number, customer, project, event, delivery_date, total, paid, balance, payment_note, invoice_footer_line, portal_link'}</small></div>
