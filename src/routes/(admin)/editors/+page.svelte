<script lang="ts">
	...
	async function reconcileEditors() {
		reconciling = true; message = '';
		const response = await fetch('/api/editors/reconcile', { method: 'POST' });
		const result = await response.json(); reconciling = false;
		if (!response.ok) { message = result.error || 'Unable to reconcile editors with Sheets'; return; }
		message = result.editorsArchived ? `${result.editorsArchived} editor(s) missing from Sheets moved to Archived.` : 'Editors already match Google Sheets.';
...
{#if showArchived}<div class="archive-context"><span><Archive size={15}/><strong>Archived editors</strong><small>Restore an editor or delete them permanently.</small></span><button class="back-active" onclick={() => { showArchived = false; detailsOpen = false; detailEditor = null; }}><ArrowLeft size={13}/> Back to active editors</button></div>{/if}
<div class="editor-tools"><button disabled={reconciling} onclick={reconcileEditors}><RefreshCw size={12} class={reconciling ? 'spin' : ''}/>{reconciling ? 'Checking…' : 'Reconcile Sheets'}</button>{#if showArchived}<button class="back-active" onclick={() => { showArchived = false; detailsOpen = false; detailEditor = null; }}><ArrowLeft size={12}/> Back to active editors</button>{:else}<button onclick={() => { showArchived = true; detailsOpen = false; detailEditor = null; }}><Archive size={12}/> View archived</button>{/if}<span>{visibleEditors.length} editors</span></div>
...
<Modal bind:open={detailsOpen} wide title={detailEditor ? `${detailEditor.name} — Editor profile` : 'Editor profile'}>
	{#if detailEditor}
		<div class="editor-detail">
			...
			{#if tasksFor(detailEditor.id).length}<div class="assignment-list">{#each tasksFor(detailEditor.id) as task}<a href={'/orders/' + task.orderId}><div class="assignment-copy"><div class="entity-tags"><span class="task-tag">Task</span><span class="customer-tag">Customer</span><span>{task.customer}</span></div><strong>{task.name}</strong><small>{task.project}{task.due ? ` · Due ${formatDate(task.due)}` : ''}</small></div><div class="assignment-status"><span>{task.status}</span><small>{task.progress}% complete</small></div></a>{/each}</div>{:else}<p class="empty-detail">No active assignments for this editor.</p>{/if}
		</div>
	{/if}
</Modal>
...
<style>
	.archive-context{display:flex;align-items:center;justify-content:space-between;gap:14px;margin:-6px 0 14px;border:1px solid #f59e0b4d;border-radius:12px;background:#f59e0b0d;padding:11px 13px}.archive-context>span{display:flex;align-items:center;gap:8px;color:#d99012}.archive-context strong{font-size:10px}.archive-context small{color:var(--muted);font-size:8px}.archive-context .back-active{height:34px;border:1px solid var(--purple);border-radius:8px;background:var(--theme-soft);color:var(--purple);font-size:9px;padding:0 10px;display:flex;align-items:center;gap:6px}.editor-tools{display:flex;align-items:center;justify-content:flex-end;gap:12px;margin-bottom:14px}.editor-tools button{height:32px;border:1px solid var(--line);border-radius:8px;background:var(--card);color:var(--muted);font-size:9px;padding:0 11px;display:flex;align-items:center;gap:6px}.editor-tools button.back-active{color:var(--purple);border-color:var(--purple);background:var(--theme-soft)}.editor-tools :global(.spin){animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.editor-tools span,.message{color:var(--muted);font-size:9px}.message{text-align:right}.editor-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;align-items:start}.editor-card{padding:18px}.editor-card.archived h2,.editor-card.archived>p,.editor-card.archived .editor-meta{text-decoration:line-through}.editor-top{display:flex;justify-content:space-between}.editor-avatar{width:39px;height:39px;border-radius:10px;background:var(--theme-soft);color:var(--purple);display:grid;place-items:center;font-size:11px;font-weight:700}.availability{display:flex;gap:5px;align-items:center;border-radius:20px;background:var(--theme-soft);color:var(--muted);font-size:8px;padding:4px 7px;text-transform:capitalize}.availability i{width:5px;height:5px;border-radius:50%;background:#94a3b8}.availability.available{color:#059669}.availability.available i{background:#10b981}.editor-card h2{font-size:13px;margin:15px 0 3px}.editor-card p{font-size:10px;color:var(--muted);margin:0}.editor-meta{border-top:1px solid var(--line);border-bottom:1px solid var(--line);margin:17px 0 13px;padding:12px 0;display:flex;justify-content:space-between;color:var(--muted);font-size:9px}.editor-meta strong{font-size:11px;margin-right:4px}.editor-actions{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(0,1fr) minmax(0,.8fr) 32px 32px;gap:7px}.editor-actions.archived-actions{grid-template-columns:minmax(0,1fr) auto}.editor-actions a,.editor-actions button{min-width:0;height:32px;border:1px solid var(--line);border-radius:8px;background:var(--card);color:var(--purple);font-size:9px;padding:0 7px;display:flex;align-items:center;justify-content:center;gap:5px;white-space:nowrap}.editor-actions .whatsapp,.editor-actions .danger{width:32px;padding:0}.editor-actions .whatsapp{color:#25d366}.editor-actions .whatsapp.unavailable{color:var(--muted);cursor:help;opacity:.55}.editor-actions .danger{color:#ef7777}.editor-detail{min-width:0}.expansion-head{display:flex;align-items:flex-start;justify-content:space-between;gap:20px}.expansion-head h3{font-size:17px;margin:7px 0 4px}.expansion-head p{margin:0;color:var(--muted);font-size:9px}.expansion-head .secondary{display:flex;align-items:center;gap:6px}.entity-tags{display:flex;align-items:center;gap:5px;flex-wrap:wrap}.entity-tags span{padding:4px 7px;border:1px solid var(--line);border-radius:999px;color:var(--muted);background:var(--card);font-size:7px;text-transform:uppercase;letter-spacing:.05em}.entity-tags .editor-tag{color:#7c3aed;border-color:#8b5cf640;background:#8b5cf612}.entity-tags .customer-tag{color:#0284c7;border-color:#0ea5e940;background:#0ea5e912}.entity-tags .task-tag{color:#059669;border-color:#10b98140;background:#10b98112}.editor-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:18px 0}.editor-stats>div{display:flex;gap:9px;border:1px solid var(--line);border-radius:10px;padding:12px;background:var(--card);color:var(--purple)}.editor-stats span{display:flex;flex-direction:column;gap:4px;color:var(--muted);font-size:8px}.editor-stats strong{color:var(--theme-text);font-size:10px;text-transform:capitalize}.assignment-head{display:flex;justify-content:space-between;align-items:center;border-top:1px solid var(--line);padding:16px 0 7px}.assignment-head span{color:var(--purple);font-size:8px;text-transform:uppercase;letter-spacing:.08em}.assignment-head strong{font-size:10px}.assignment-list{display:grid}.assignment-list>a{display:flex;justify-content:space-between;align-items:center;gap:15px;border-top:1px solid var(--line);padding:12px 0}.assignment-copy{display:flex;flex-direction:column;gap:5px;min-width:0}.assignment-copy>strong{font-size:11px}.assignment-copy>small{font-size:8px;color:var(--muted)}.assignment-status{display:flex;flex-direction:column;align-items:flex-end;gap:4px}.assignment-status>span{font-size:9px;color:var(--purple)}.assignment-status>small,.empty-detail{font-size:8px;color:var(--muted)}.empty-detail{margin:14px 0 0}@media(max-width:1050px){.editor-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:650px){.archive-context{align-items:flex-start;flex-direction:column}.archive-context>span{align-items:flex-start;flex-wrap:wrap}.editor-grid{grid-template-columns:1fr}.editor-stats{grid-template-columns:1fr 1fr}.expansion-head{flex-direction:column}.assignment-list>a{align-items:flex-start}.assignment-status{min-width:82px}}@media(max-width:430px){.editor-actions{grid-template-columns:repeat(5,1fr)}.editor-actions.archived-actions{grid-template-columns:repeat(2,1fr)}.editor-actions .whatsapp,.editor-actions .danger{width:100%}.action-label{display:none}.editor-stats{grid-template-columns:1fr}}
