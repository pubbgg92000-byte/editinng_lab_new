...
export const PATCH = async ({ params, request, cookies, locals }) => {
	...
	try {
		if (input.customFields !== undefined) {
			if (hasCapability(configuration.effectiveCapabilities, 'customFields')) input.customFields = validateCustomValues(configuration.profile, 'staff', input.customFields);
...
export const POST = async ({ params, request, cookies, locals }) => {
	...
	const configuration = await getTenantConfiguration(database, locals.tenant!);
	if (!hasCapability(configuration.effectiveCapabilities, 'work.staffPortal')) return json({ error: 'Staff portal is not available for this workspace.' }, { status: 403 });
