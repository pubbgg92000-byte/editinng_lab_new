// Builds a current editor assignment WhatsApp link for tasks on one order.
import { json } from '@sveltejs/kit';
import { verifySession } from '$lib/server/auth';
import { readyDatabase } from '$lib/server/db';
import { getBusinessProfile, getOrder, getSettings, listEditors, recordActivity, regenerateEditorToken } from '$lib/server/repository';
import { editorAssignmentMessage, normalizePhone, whatsappUrl } from '$lib/server/whatsapp';
...
export const POST = async ({ params, request, cookies, locals, url }) => {
	...
	if (!normalizePhone(editor.phone)) return json({ error: 'Add a valid 10-digit worker WhatsApp number first.' }, { status: 400 });
	const configuration = await getTenantConfiguration(database, locals.tenant!);
	const templateId = String(input.templateId || '') || undefined;
	if (!templateForScenario(configuration.moduleConfiguration, 'work-assignment', templateId)) return json({ error: 'That WhatsApp template is unavailable.' }, { status: 400 });
	...
	const message = editorAssignmentMessage(settings, editor, order, tasks, token, url.origin, locals.tenant!.slug, profile, configuration.moduleConfiguration, templateId);
	...
	return json({ ok: true, message, url: whatsappUrl(editor.phone, message), token });
};
