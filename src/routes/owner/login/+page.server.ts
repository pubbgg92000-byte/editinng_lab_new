...
export const actions = {
	default: async ({ request, cookies, url, getClientAddress }) => {
		...
		if (!result.account || result.account.role !== 'owner') return fail(400, { invalid: true, email });
		const session = await createAuthSession(result.account);
		cookies.set(SESSION_COOKIE, session.token, { path: '/', httpOnly: true, sameSite: 'lax', secure: url.protocol === 'https:', maxAge: session.maxAge });
		redirect(303, '/owner');
	}
};
