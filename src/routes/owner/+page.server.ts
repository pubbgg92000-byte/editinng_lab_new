...
export const actions = {
	...
	connections: async ({ request, locals }) => {
		...
		const databaseUrl = field(form, 'databaseUrl') || tenant.databaseUrl;
	...
	credentials: async ({ request, locals }) => {
		const form = await request.formData();
		try {
			await updateTenantCredentials(field(form, 'tenantId'), field(form, 'email'), field(form, 'password'), locals.account!.id);
			return { action: 'credentials', success: 'Credentials updated and existing sessions revoked.' };
	...
	ownerPassword: async ({ request, locals, cookies }) => {
		const form = await request.formData();
		try { await changeOwnerPassword(locals.account!.id, field(form, 'password')); }
