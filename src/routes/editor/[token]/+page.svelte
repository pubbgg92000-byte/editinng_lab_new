...
<main class="portal-main">
	{#if !selected}
		...
		<div class="work-title"><h2>Active assignments</h2><span>{tasks.length} assigned</span></div>
		{#if tasks.length}
			<div class="task-cards">
				{#each tasks as task}
					<button class="task-card" onclick={() => openTask(task)}>
						...
						<p>{task.name}</p>
						<div class="task-footer"><span><CalendarDays size={13}/> {task.due ? `Due ${task.due}` : 'No due date'}</span><span class="open">Open <ArrowUpRight size={13}/></span></div>
					</button>
				{/each}
			</div>
	...
	{:else}
		...
		<div class="task-layout">
			<section class="card details">
				<h2>Work details</h2>
				<dl><div><dt>Task</dt><dd>{selected.name}</dd></div><div><dt>Due date</dt><dd>{selected.due || 'Not set'}</dd></div><div><dt>Event</dt><dd>{selected.workType || '—'}</dd></div></dl>
			...
			<section class="card update">
				...
				<div class="field"><label for="task-output">Output link</label><div class="with-icon"><FileUp size={14}/><input id="task-output" bind:value={outputLink} placeholder="Paste a Drive or delivery link"/></div></div>
